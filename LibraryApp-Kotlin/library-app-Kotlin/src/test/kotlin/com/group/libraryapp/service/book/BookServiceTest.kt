...
class BookServiceTest @Autowired constructor(
    ...
    assertThat(result[0].name).isEqualTo("책이름")
    ...
    bookRepository.save(Book.fixture("책이름"))
        val savedUser = userRepository.save(User("둘리", null))
        val request = BookLoanRequest("둘리","책이름")
    ...
    // then
        ...
        assertThat(result).hasSize(1)
        assertThat(result[0].bookName).isEqualTo("책이름")
    ...
    fun returnBookTest(){
        bookRepository.save(Book.fixture("책이름"))
        val saveUser = userRepository.save(User("둘리", null))
        userLoanHistoryRepository.save(
            UserLoanHistory.fixture(
                saveUser,
                "책이름"
        ...
        val request = BookReturnRequest("둘리", "책이름")
    ...
    // given
        val savedUser = userRepository.save(User("둘리", null))
        val results = userLoanHistoryRepository.saveAll(
            listOf(
    ...
    // when
    val results = bookService.getBookStatus()
    // then
